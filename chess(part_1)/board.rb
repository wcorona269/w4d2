...
class Board
    ...
    def initialize
        ...
        @rows = Array.new(8) {Array.new(8, Null_Piece.new )}
    end
    ...
    def move_piece(start_pos, end_pos)
        x, y = end_pos 
        if valid_pos?(end_pos) && start_pos != end_pos && !self[start_pos].is_a?(Null_Piece)
            this_piece = self[start_pos]
            self[end_pos] = this_piece
            self[start_pos] = Null_Piece.new
        else
